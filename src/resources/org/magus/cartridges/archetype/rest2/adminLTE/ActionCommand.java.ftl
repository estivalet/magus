...
package ${app.shortName}.server.commands;
 ...
 * @author ${app.user}
 */
public class ${clazz.getAlias(true)}ActionCommand implements ICommand {
	...
	public Object execute_old(HttpServletRequest request, HttpServletResponse response, IContext context) throws Exception {
     ...
     * @param response
     */
    private void filter_old(HttpServletRequest request, HttpServletResponse response) throws Exception {
        int pageNum = 1;
	 ...
	 * @throws Exception
	 */
	private ${clazz.getAlias(true)}Model populateDto_old(HttpServletRequest request,${clazz.getAlias(true)}Model model) throws Exception {
        <#list allColumns as column><#t>
        <#if (column.getPSJavaMethod() == "Bytes")>
