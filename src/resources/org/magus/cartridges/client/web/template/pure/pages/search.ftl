...
function filterSearch(pageNum) {
    ...
    callServer("?command=${clazzDomainName}Action&action=filter" + query + filterParams, filterSearchStatus);
}
...
window.onload = function() {
    ...
    if(firstTime == "Y") {
        callServer("?command=${clazzDomainName}Action&action=filter&pageNum=1", filterSearchStatus);
    }
