...
function filterSearchStatus() {
    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
        ...
        <#list columns as column><#t>
            ...
            cell${column_index}.innerHTML = "<a href=\"#createModal\" onclick=\"edit${clazzName?capitalize}('" + ${column.getCamelCaseName()} + "')\">" + ${column.getCamelCaseName()} + "</a>";
            ...
            cell${columns?size}.innerHTML = "<a href=\"#createModal\" onclick=\"edit${clazzName?capitalize}('" + ${pkColumn} + "')\"><img src=\"css/pimpa_yes.gif\" alt=\"picture\" class=\"tabpimpa\" height=\"13\" width=\"13\"></a>";
            cell${columns?size}.innerHTML += "&nbsp;<a href=\"#\" onclick=\"remove${clazzName?capitalize}('" + ${pkColumn} + "')\"><img src=\"css/pimpa_no.gif\" alt=\"picture\" class=\"tabpimpa\" height=\"13\" width=\"13\"></a>";
        }
...
function filterDetailStatus() {
    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
        ...
        <#list fkTableColumns as column><#t>
            ...
            <#else>
            <#if (column.isColumnInPrimaryKey())>
            cell${column_index}.innerHTML = "<a href=\"#\" onclick=\"edit('','${fks.getFkCamelCaseName(true)}Action','update','" + ${column.getCamelCaseName()} + "')\">" + ${column.getCamelCaseName()} + "</a>";
            <#assign pkColumn="${column.getCamelCaseName()}">
            <#else>            
            cell${column_index}.innerHTML = ${column.getCamelCaseName()};
            </#if>
            ...
            cell${fkTableColumns?size}.innerHTML = "<a href=\"#\" onclick=\"edit('','${fks.getFkCamelCaseName(true)}Action','update','" + ${pkColumn} + "')\"><img src=\"css/pimpa_yes.gif\" alt=\"picture\" class=\"tabpimpa\" height=\"13\" width=\"13\"></a>";
            cell${fkTableColumns?size}.innerHTML += "&nbsp;<a href=\"#\" onclick=\"remove${clazzName?capitalize}('" + ${pkColumn} + "')\"><img src=\"css/pimpa_no.gif\" alt=\"picture\" class=\"tabpimpa\" height=\"13\" width=\"13\"></a>";
        }
        setUpPagination(obj.page, Math.ceil(obj.total / 5));  
    }
}
...
function filter(pageNum) {
    callServerJSON("GET","http://localhost:8080/${appShortName}/rest/${clazzName}/all/json?pageNum=" + pageNum, filterSearchStatus);
}
...
function edit${clazzName?capitalize}(${pkColumn}) {
    callServerJSON("GET","http://localhost:8080/${appShortName}/rest/${clazzName}/" + ${pkColumn} + "/json", edit${clazzName?capitalize}Status);    
}

function edit${clazzName?capitalize}Status() {
    if (xmlhttp.readyState == 4 && xmlhttp.status == 200) {
        var obj = JSON.parse(xmlhttp.responseText);
        <#list columns as column><#t>
...
function remove${clazzName?capitalize}(${pkColumn}) {
    alertify.confirm('Confirm exclusion?', function () {
        var data = {"${pkColumn}":${pkColumn}};
        callServerJSON("DELETE","http://localhost:8080/${appShortName}/rest/${clazzName}/delete", remove${clazzName?capitalize}Status, data);
    });  
}

function remove${clazzName?capitalize}Status() {
    if (xmlhttp.readyState == 4 && xmlhttp.status == 200){
        document.getElementById("messageIdAlert").style.display = "block";
        document.getElementById("messageAlert").innerHTML = xmlhttp.responseText;
        filter(1);
    ...
    <div class="container">
        <div class="table-container">
            <div class="table-header">
                <div class="table-header-text">List of ${clazz}&nbsp;<a href="#createModal"><img style="vertical-align:text-top" src="css/plus.png"/></a></div>
            </div>
