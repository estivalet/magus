...
function checkConnection() {
	callServer("magus?command=Database&action=checkConnection&database=" +document.getElementById('database').value + "&url=" + document.getElementById('dbURL').value+"&username="+document.getElementById('dbUser').value+"&password="+document.getElementById('dbPass').value, checkConnectionStatus);
}
...
function newProject() {
    ...
    callServer("magus?command=Project&action=new&projectName=" + projectName + "&shortName=" + projectShortName + "&description="+projectDescription + "&path="+projectPath + "&template=" + template, newProjectStatus);
}
...
function saveProject() {
	...
	callServer("magus?command=Project&action=save&id=" + id + "&database=" + database + "&dbURL="+dbURL + "&dbUser="+dbUser + "&dbPass=" + dbPass, saveProjectStatus);
}
...
function listSchemas() {
    callServer("magus?command=Database&action=listSchemas&database=" +document.getElementById('database').value + "&url=" + document.getElementById('dbURL').value+"&username="+document.getElementById('dbUser').value+"&password="+document.getElementById('dbPass').value, listSchemasStatus);
}
...
function listTableColumns(table) {
	if(table != "") {
		callServer("magus?command=Database&action=listTableColumns&id="+document.getElementById('id').value + "&schema=" + document.getElementById('schema').value + "&table=" + table, listTableColumnsStatus);
	}
}
...
function listTableColumnsStatus() {
	if (HttpReq.readyState == 4 && HttpReq.status == 200){
    	...
    	for(i=0; i < xmlData.length; i++) {
    		...
    		addRow("columnTbl",xmlData[i].getAttribute("name"),label, columnType, displayOrder, comment, masked, visible, filter);
		}
	}
}

function addRow(tableID, columnName, label, columnType, displayOrder, comment, masked, visible, filter) {
    ...
    element4.setAttribute('onclick','checkColumnType(' + "'" + tableID + "', " + (rowCount-1) + ');');
...
function checkColumnType(tableID, row) {
	...
	query += "&column="+column;
	callServer("magus?command=Database&action=listColumnOptions"+query, checkColumnTypeStatus);
...
function checkColumnTypeStatus() {
	if (HttpReq.readyState == 4 && HttpReq.status == 200){
        ...
        document.getElementById('optionText').onkeydown = function(e){
     	   if(e.keyCode == 13){
     		   addOption(document.getElementById('optionValue').value,document.getElementById('optionText').value,true);
     	   }
     	};
...
function deleteOption() {
    ...
    for(var i=0; i<rowCount-1; i++) {
    	if(table.rows[i].cells[2].firstChild.checked) {
    		ids.push(i);
    	}
    }
...
function saveOptions() {
	...
	callServer("magus?command=Database&action=saveColumnOptions"+query, saveOptionsStatus);
...
function saveColumnMapping() {
	...
	callServer("magus?command=Database&action=saveColumnMapping"+query, saveColumnMappingStatus);
}
...
function generateCode() {
	openModal();
	callServer("magus?command=Generate&project="+document.getElementById('project').value+"&shortName=" + document.getElementById('shortName').value+"&name=" + document.getElementById('name').value+"&path=" + document.getElementById('path').value + "&template=" + document.getElementById("template").value, generateCodeStatus)
}
